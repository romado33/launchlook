...
| 2026-05-26 | q15 shipped: `form_submit_smoke` finding category (form-submit smoke test runner backing The Stranger Who Tried to Sign Up). New runner at `scripts/ai_audit/form_smoke_test.py` detects forms, fills with safe synthetic values, submits, and watches for confirmation. Checkout / payment / destructive forms are detected and skipped on purpose (surfaced as a "we saw it but didn't submit" finding so the customer knows). Pro-tier-only email round-trip via `scripts/ai_audit/disposable_mailbox.py` (mail.tm, falls back gracefully). Customer YAML opt-out at `form_smoke_test.enabled: false` plus a `blocked_forms` selector blocklist. Buyer-facing name per `SIMPLICITY-GUARDRAILS.md` §6: "form & signup flows". Internal jargon ("form-submit smoke test", "synthetic values", "round-trip") stays inside dev docs and YAML `display_name_internal` only. The Stranger Who Tried to Sign Up persona was already in `TESTERS-CAST.md` (added in q5+q13); q15 wires the backend runner that gives that persona real findings to attach to. |
| 2026-05-26 | q22 shipped: hosted shareable report pages at `launchlook.app/r/{slug}`. Every delivered audit gets a per-customer URL backed by `landing/r/{slug}.html` (with Open Graph + Twitter Card meta tags baked in at delivery time) and `landing/data/reports/{slug}.json` (the page content). Default is **private** per `SIMPLICITY-GUARDRAILS.md` §3 and §5 (no surprises, respect customer URLs). Customer opts in by replying "share" to the delivery email; Rob flips with `python scripts/share_report.py --slug X --public`. The publish path runs a strict sanitizer (`scripts/sanitize_for_public.py`) that strips the customer URL, email, screenshots, internal CRM keys, and translates common path names into generic phrases. Pro tier customers can independently opt in to expose the q18 Handoff Report via `--share-handoff`. The public page renders the q17 Verified banner inline so the badge → report → CTA loop closes on one surface. One FAQ entry added to `landing/index.html` and `landing/webflow.html`; not a pricing card feature, not in main nav. Full workflow at `docs/SHAREABLE-REPORT-WORKFLOW.md`. |
| 2026-05-26 | q-final-audit shipped: internal consistency audit + auto-safe fixes + `docs/CONSISTENCY-AUDIT-REPORT.md`. New reusable scanner at `scripts/consistency_check.py` checks every customer-facing surface (`landing/`, `customers/`, `templates/email/`, `templates/report/`, `templates/qsg/`, `templates/handoff/`, `templates/confidence_check/`, `templates/r/`) against the canonical tier ladder + finding caps in §1 / §7, the forbidden vocabulary list in `SIMPLICITY-GUARDRAILS.md` §6, the persona spelling in `TESTERS-CAST.md`, and the buyer-facing finding categories in `scripts/ai_audit/finding_categories.yaml`. Also sanity-checks the Stripe cents-to-tier routing in `api/stripe-webhook.py` (Starter $19 / Scale Up $49 / Pro $99 + metadata-routed Confidence Check / re-verify / Handoff Report add-ons). The first run applied **19 string-replace renames** (`Full Package` → `Scale Up Package` and variants across `landing/index.html` + `landing/privacy.html` + `landing/terms.html`) plus a **13-em-dash judgement pass** (prose em-dashes rewritten as comma / colon / period across `landing/checklist.html`, `landing/index.html`, `landing/thanks-free-audit.html`, `templates/handoff/handoff.md.j2`). Remaining 56 deltas are human-review (forbidden-vocab leaks like "Comprehensive audit" / "Priority triage", internal taxonomy leaks like "Trust gaps" / "Cross-user data check", stale finding caps `7`/`25` instead of canonical `10`/`30`, and the 3 prose em-dashes whose surrounding sentences need the same rewrite). All listed in `ROB-REMAINING-TODO.md` for the morning sweep. Stripe routing is consistent across the webhook, no critical issues. q-final-lint can proceed. |
| 2026-05-26 | q-final-lint shipped (final pre-launch worker): linted Python (`ruff` + `black` clean across `scripts/`, `tests/`, `api/`, `lib/`; `mypy` clean across `scripts/ai_audit/`) + frontend (`prettier` applied to `landing/assets/*.js`; HTML left alone deliberately so `prettier` defaults didn't churn the existing 2-space style — documented in `REFACTOR-NOTES.md`) + light refactor (ambiguous `l` rebound to `link` / `label` per `ruff E741`, dead `verdict` / `needs_review_lines` removed per `ruff F841`, BeautifulSoup attribute accesses cast to `str` for `mypy union-attr`, `seen_keys` retyped to `set[tuple[str, str]]`, `Counter` annotations added in `feedback_summary.py`, `UnicodeEncodeError` in `scripts/share_report.py` fixed by `sys.reconfigure(encoding="utf-8")` to match `scripts/deliver_report.py`) + fixed q18 Handoff Report CLI regression (`scripts/deliver_report.py` now calls `ai_pipeline.run_handoff_report(audit_payload=data, ...)` with keyword args — the function in `scripts/ai_audit/pipeline.py` was already keyword-only from q18's original commit `473e891`; also added `sys.path.insert(0, REPO_ROOT)` so the script imports cleanly from any cwd) + completed forbidden-vocab sweep on customer-facing surfaces (4× "Comprehensive audit" rewritten to "Ready for real users" / "Full audit", 5× "Priority triage" rewritten to "The 10 things to fix first" / "The 10 most important findings", 2× "safe synthetic values" rewritten to "safe test data" — all in `landing/index.html` + `landing/webflow.html`, with stale Starter cap `7` and Scale Up cap `25` bumped to the canonical `10` / `30` from §1) + em-dash sweep (3 prose em-dashes eliminated naturally via the tier-card copy rewrites; 16 UI placeholder em-dashes in `landing/index.html` table cells + `landing/r.html` + `templates/r/shareable.html.j2` swapped to ASCII `-` for consistency) + full smoke-test pass (`deliver_report.py` across all 3 example customers, q18 `--handoff-report --provider stub` produces both `handoff-report.md` and `handoff-report.pdf`, q6 `confidence_check.py`, q17 `generate_verified_badge.py`, q22 `share_report.py` status/public/private toggle, `ai_costs_report.py --summary`, q19 `github_push.py --dry-run`, `consistency_check.py --report-only` shows 0 critical issues, `pytest tests/` 124/124 passing). Full diff summary at `docs/REFACTOR-NOTES.md`. Cites `SIMPLICITY-GUARDRAILS.md` §6 and the punch list in `docs/CONSISTENCY-AUDIT-REPORT.md`. Repo is shipping-ready. |
