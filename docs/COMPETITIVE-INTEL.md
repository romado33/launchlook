...
| **VibeDoctor** ([vibedoctor.io](https://vibedoctor.io)) | "Production readiness platform for AI-coded apps. 129+ checks in 30s." | Free (3 scans/day) / Watch / Guard / Shield — entry **$15/mo** per PR-release; team plans up to 25 seats | Public PR launch May 6, 2026 (PRLog); ships from Patna, India; testimonials on pricing page (low-volume but real). Listed on multiple aggregator sites. Solo founder origin story. | "AI-specific" checks (hallucinated imports, god files, empty tests); free continuous scanning; GitHub PR integration; rich visual "Vibe X-Ray" | **Requires GitHub repo** for most of its 129 checks — URL-only path yields only ~8 (performance, SEO, security headers, SSL, console errors). Most LaunchLook customers are Lovable/Bolt users who have no repo to connect. Subscription-only, no one-off purchase. | Indie hackers + teams with a GitHub repo shipping with Cursor/Copilot/Bolt | **Complementary, not competing** (updated May 28, 2026 after deeper analysis). VibeDoctor = code-level scanner (secrets, CVEs, AST). LaunchLook = live-UX auditor (form submits, emails, mobile layout). Most serious founders should run both. Positioning opportunity: a LaunchLook page titled "What scanners miss" covering VibeDoctor + PageLens. | LaunchLook: URL-only (no repo), human-live-workflow testing, one-time cost. Framing: "VibeDoctor checks the code. LaunchLook walks the app." |
