...
This folder contains the files for the standalone public checklist repo. It's intentionally separate from the main `onceover` repo so the public-facing artifact stays clean (no internal docs, no scripts, no fix prompts).

## One-time setup

```bash
# From the onceover repo root:
cd external/onceover-prelaunch-checklist
...
gh repo create onceover-prelaunch-checklist \
  --public \
...
The repo URL becomes `https://github.com/{your-handle}/onceover-prelaunch-checklist`.

## Updating

When the in-app `/checklist` page changes:

1. Update `external/onceover-prelaunch-checklist/README.md` to match (verbatim — no marketing words sneaking in)
...
| Location | File |
|----------|------|
| Public GitHub repo | `external/onceover-prelaunch-checklist/README.md` |
| Onceover website /checklist page | `landing/checklist.html` |
...
- Keeping it separate means anyone can star/fork/contribute without touching internal Onceover stuff.
- It also lets you point to a stable URL: `github.com/{your-handle}/onceover-prelaunch-checklist`.

## What NOT to publish

The main `onceover` repo contains:
